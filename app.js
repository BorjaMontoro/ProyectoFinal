...
// Init objects
db.init({
  host: process.env.MYSQLHOST || "containers-us-west-29.railway.app",
  port: process.env.MYSQLPORT || 8074,
  user: process.env.MYSQLUSER || "root",
  password: process.env.MYSQLPASSWORD || "jCObGkfu3WiJ1Ohb3byO",
  ...
  if (receivedPOST) {
    if(isNaN(receivedPOST.direccion)){
      if (receivedPOST.direccion.trim()==""){
        ...
        let regex = /^[ a-zA-ZñÑàèòáéíóúÁÉÍÓÚ,\-\d]+$/;
        if (regex.test(receivedPOST.direccion)){
          ...
          result = {status: "ERROR", message: "La direccion no puede contener comillas entre otros caracteres especiales"}
        }
      }
    }else{
